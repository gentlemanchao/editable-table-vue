 ...
 * @param {String} value 
 */
let getJsonArrayIndexByKeyValue = function(arr, key, value) {
    let _d, i;
    for (i = 0; i < arr.length; i++) {
        _d = arr[i];
        if (typeof(_d[key]) === undefined) continue;
        if (_d[key] == value) {
            return i;
        }
    }
...
/**
 * 获取dom指定选择器的所有children节点
 * @param {Object} dom 
 * @param {String} selector 
 */
let getDomChildren = function(dom, selector) {
 ...
 */
let getDomParents = function(dom, parentNodeName, parentClass) {
    let _dom, _parent, i;
    _dom = dom;
    for (i = 0; i < 100; i++) {
        _parent = _dom.parentNode;
        if (_parent.isEqualNode(_dom.ownerDocument)) {
            return null;
        }
        if (judgeNodeByNameAndClass(_parent, parentNodeName, parentClass)) {
            return _parent;
        } else {
            _dom = _parent;
        }
...
export default  {
    ...
    getIndexByKeyValue: getJsonArrayIndexByKeyValue, //获取json数组中指定数据的索引
}